...
match_score = dis12(1,:)./dis12(2,:);
match_score = match_score((dis12(1,:)./dis12(2,:))<0.9^2);

% Find unique matches
[match_score, idx] = sort(match_score, 'ascend');
match = match(:, idx);
[~, uniq_idx, ~] = unique(match(2, :), 'first');
match = match(:, uniq_idx);
...
while ii<Nh
  ii=ii+1;   
  [inls,H{ii}] = geometricVerification(f1, f2, mt, ...
